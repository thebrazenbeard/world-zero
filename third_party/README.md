...
A wheelhouse is not trusted merely because files exist. Its `MANIFEST.json` must pass `python tools/verify_wheelhouse.py <wheelhouse>`, and an offline qualification must install into a fresh virtual environment using `python tools/bootstrap_offline.py --wheelhouse <wheelhouse> --venv <path>`.
