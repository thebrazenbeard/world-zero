...
`pyproject.toml` declares the direct dependency contract. `uv.lock` records the exact connected resolution. Platform-specific wheelhouses under `third_party/wheels/` provide install artifacts when package indexes are unavailable.
