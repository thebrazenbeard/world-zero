"""Evidence-source lineage tracking for independence-aware validation."""
...
class LineageRegistry:
    ...
    def get(self, source_id: str) -> LineageRef:
        try:
            ...
            raise KeyError(f"unknown source lineage: {source_id}") from exc

    def independent_source_count(self, source_ids: set[str]) -> int:
        roots = {self.get(source_id).upstream_lineage_id for source_id in source_ids}
        return len(roots)
