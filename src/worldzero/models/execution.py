...
def execute_baseline_to_files(
    *,
    ...
    receipt = RuntimeReceipt(
        ...
        scenario=ArtifactBinding(path=scenario_path.as_posix(), sha256=_sha256(resolved_scenario)),
        data_bundle=ArtifactBinding(path=data_bundle_path.as_posix(), sha256=_sha256(resolved_bundle)),
        parameter_set=ArtifactBinding(
            path=parameter_set_path.as_posix(),
            sha256=_sha256(resolved_parameters),
        ),
        region_set=ArtifactBinding(
            path=region_set_path.as_posix(),
            sha256=_sha256(resolved_region_set),
        ),
        cohort_set=ArtifactBinding(
            path=cohort_set_path.as_posix(),
        ...
        result=ArtifactBinding(path=output_path.as_posix(), sha256=result_sha256),
    )
